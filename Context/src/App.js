...
import { Postprovider, PostContext } from "./context/Postprovider";
...
function List() {
   const { posts, searchedPosts } = useContext(PostContext);
  return (
    <ul>
      {posts.map((post, i) => (
        <li key={i}>
...
function Archive() {
  const { createRandomPost, handleAddPost } = useContext(PostContext);
